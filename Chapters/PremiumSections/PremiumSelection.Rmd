...
where $\mathrm{I}(\cdot)$ is the indicator function, returning a 1 if the event is true and zero otherwise. For a given value $s$, $\hat{F}_P(s)$ gives the proportion of premiums less than or equal to $s$, and $\hat{F}_{L}(s)$ gives the proportion of losses for those policyholders with premiums less than or equal to $s$. The graph $\left(\hat{F}_P(s),\hat{F}_{L}(s) \right)$ is known as a **concentration curve**.
