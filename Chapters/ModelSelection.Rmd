...
Now consider the function $\mathrm{g}(X) = I(X \le x)$ for a fixed $x$. Here, the notation $I(\cdot)$ is the `r Gloss('indicator')` function; it returns 1 if the event $(\cdot)$ is true and 0 otherwise. For this choice of $\mathrm{g}(\cdot)$, the expected value is $\mathrm{E~} I(X \le x) = \Pr(X \le x) = F(x)$, the distribution function evaluated at $x$. Using the `r Gloss('analog principle')`, we define the nonparametric estimator of the distribution function
...
\end{aligned}
$$
As $F_n(\cdot)$ is based on only observations and does not assume a parametric family for the distribution, it is nonparametric and also known as the `r Gloss('empirical distribution function')`.
...
{\small
\begin{matrix}
\text{Table 4.1: Popular Kernel Choices}\\
\begin{array}{l|cc}
\hline
\text{Kernel} &  w(x) \\
\hline
...
```{r PPPlot, message=FALSE, warning=FALSE, echo=FALSE, fig.cap='Probability-Probability ($pp$) Plots. The horizontal axis gives the empirical distribution function at each observation. In the left-hand panel, the corresponding distribution function for the gamma is shown in the vertical axis. The right-hand panel shows the fitted Pareto distribution. Lines of $y=x$ are superimposed.', out.width='80%', fig.asp=0.60, fig.align='center'}
#  PP Plot
par(mfrow=c(1, 2))
...
**Example  `r chapnum`.1.7. Actuarial Exam Question (modified).**
...
Solving for each parenthetical expression gives $\frac{2}{3}=(89.2/\theta)^{\hat{\gamma}}$ and $4=(206/{\hat{\theta}})^{\hat{\gamma}}$. Taking the ratio of the second equation to the first gives $6=(206/89.2)^{\hat{\gamma}}\Rightarrow {\hat{\gamma}}=\frac{\ln(6)}{\ln(206/89.2)} = 2.1407$. Then $4^{1/2.1407}=206/{\hat{\theta}} \Rightarrow {\hat{\theta}}=107.8$.
...
$$
  L(\theta) =  - \sum_{i=1}^n \left((1-\delta_i) \ln \theta + \frac{x_i^{\ast \ast}}{\theta}\right)
  (\#eq:EXPloglik)
$$
...
**Joint Distribution.** The distribution of outcomes and model parameters is known as the joint distribution and denoted as $f(x , \boldsymbol \theta) = f(x|\boldsymbol \theta )\pi(\boldsymbol \theta)$.
...
\end{aligned}
$$
Because a *pdf* integrates to one, we can calculate the propotionality constant as
$$
\begin{aligned}
...
For conjugate distributions, the posterior and the prior belong to the same family of distributions. The following illustration looks at the Poisson-gamma special case, the most well-known in actuarial applications.
