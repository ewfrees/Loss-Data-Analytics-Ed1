...
```{r sampleani1, animation.hook='gifski', eval = ANIMATION, echo=SHOW_PDF}
...
$$
This can be related to the `r Gloss('importance sampling mechanism')` : we draw more frequently in regions where we expect to have quantities that have some interest. So we consider a change of measure.

```{r sampleani_IS_1, animation.hook='gifski', eval = ANIMATION, echo=SHOW_PDF}
#install.packages('gifski')
#if (packageVersion('knitr') < '1.20.14') {
#  remotes::install_github('yihui/knitr')
#}
...
```{r sampleani_IS_2, animation.hook='gifski', eval = ANIMATION, echo=SHOW_PDF}
pic_ani = function(){
  u=seq(0,5,by=.01)
  plot(u,pnorm(u,2.5,1),col="white",ylab="",xlab="")
  rect(-1,-1,6,2,col=rgb(1,0,0,.2),border=NA)
