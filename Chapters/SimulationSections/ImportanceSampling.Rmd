...
```{r sampleani1, animation.hook='gifski', eval = ANIMATION, echo=ShowCode()}
...
```{r sampleani_IS_1, animation.hook='gifski', eval = ANIMATION, echo=ShowCode()}
#install.packages('gifski')
#if (packageVersion('knitr') < '1.20.14') {
#  remotes::install_github('yihui/knitr')
#}
...
```{r sampleani_IS_2, animation.hook='gifski', eval = ANIMATION, echo=ShowCode()}
pic_ani = function(){
  u=seq(0,5,by=.01)
  plot(u,pnorm(u,2.5,1),col="white",ylab="",xlab="")
  rect(-1,-1,6,2,col=rgb(1,0,0,.2),border=NA)
