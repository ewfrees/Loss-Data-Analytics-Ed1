...
```{r comment="", warning=FALSE, echo=ShowCode()}
...
$$
```{r BootstrapDistn, fig.cap='Distribution of Bootstrap Replicates. The left-hand panel is a histogram of replicates. The right-hand panel is a quantile-quantile plot, comparing the bootstrap distribution to the standard normal distribution.', fig.align='center',  comment="", echo=ShowCode()}
library(boot)
...
```{r BootstrapCompare, fig.cap='Comparision of Estimates. True value of the parameter is given by the solid horizontal line at 5.20.', fig.align='center',  comment="", echo=ShowCode()}
set.seed(2074)
ns<- 200
est <- function(n){
...
```{r CoefVarCompare, fig.cap='Bootstrap Distributions of the Coeffiecient of Variation.', fig.align='center',  comment="",echo=ShowCode()}
set.seed(2074)
CV <- matrix(NA,1e5,2)
for(s in 1:nrow(CV)){
