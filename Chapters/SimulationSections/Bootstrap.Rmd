...
There are two basic resampling methods, *model-free* and *model-based*, which are also known, respectively, as *nonparametric* and *parametric*. In the `r Gloss('nonparametric approach')`, no assumption is made about the distribution of the parent population. The simulated draws come from the empirical distribution function $F_n(\cdot)$, so each draw comes from $\{X_1, \ldots, X_n\}$ with probability 1/*n*. 
...
The bootstrap can be used to quantify the bias of an estimator, for instance. Consider here a sample $\mathbf{x}=\{x_1,\cdots,x_n\}$ `r Gloss('iid')` with mean $\mu$.

```{r comment="", echo=SHOW_PDF}
sample_x <- c(2.46,2.80,3.28,3.86,2.85,3.67,3.37,3.40,5.22,2.55,2.79,4.50,3.37,2.88,1.44,2.56,2.00,2.07,2.19,1.77)
```

Suppose that the quantity of interest is $\theta=\exp[\mu]$. A `r Gloss('natural estimator')` would be $\widehat{\theta}_1=\exp(\overline{x})$. This estimator is biased (`r Gloss('Jensen inequality')`) but asymptotically unbiased.
...
$$
```{r BootstrapDistn, fig.cap='Distribution of Bootstrap Replicates. The left-hand panel is a histogram of replicates. The right-hand panel is a quantile-quantile plot, comparing the bootstrap distribution to the standard normal distribution.', fig.align='center',  comment="", echo=SHOW_PDF}
library(boot)
...
```{r CoefVarCompare, fig.cap='Bootstrap Distributions of the Coeffiecient of Variation.', fig.align='center',  comment="",echo=SHOW_PDF}
set.seed(2074)
CV <- matrix(NA,1e5,2)
for(s in 1:nrow(CV)){
